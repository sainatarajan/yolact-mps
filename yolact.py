...
# Device detection for Mac M4 Pro
...
use_jit = (torch.cuda.device_count() if torch.cuda.is_available() else 0) <= 1
if not use_jit:
    ...
    def make_priors(self, conv_h, conv_w):
        ...
        with timer.env('makepriors'):
            if self.last_conv_size != (conv_w, conv_h):
                ...
                # CHANGE: Create tensor on the correct device
                self.priors = torch.Tensor(prior_data).view(-1, 4).to(device)
                self.last_conv_size = (conv_w, conv_h)
    ...
    def __init__(self):
        ...
        if cfg.mask_type == mask_type.direct:
            ...
            if cfg.mask_proto_use_grid:
                ...
                self.grid = torch.Tensor(np.load(cfg.mask_proto_grid_file)).to(device)
    ...
    def init_weights(self, backbone_path):
        ...
        for name, module in self.named_modules():
            ...
            try:
                ...
                # WeakScriptModuleProxy doesn't exist in PyTorch 1.7.1
                # Fallback to checking if it's a Conv layer by name/type
                is_script_conv = False
                try:
                    if 'Conv' in str(type(module)):
                        # Check if it has the expected constants if they exist
                        if hasattr(module, '__dict__') and '_constants_set' in module.__dict__:
                            is_script_conv = all_in(module.__dict__['_constants_set'], conv_constants) \
                                            and all_in(conv_constants, module.__dict__['_constants_set'])
                        else:
                            # Simple fallback - just check if it's a conv layer
                            is_script_conv = any(conv_type in str(type(module)) 
