...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    #     device_key = str(on_gpu) if on_gpu is not None else 'cpu'
    #     if on_gpu is not None and color_idx in color_cache[device_key]:
    #         return color_cache[device_key][color_idx]
    #     else:
    ...
    #             device_key = str(on_gpu)
    #             color_cache[device_key][color_idx] = color
    #         else:
    #             color = torch.Tensor(color).float() / 255.
    #         return color

    def get_color(j, on_gpu=None, return_tensor=False):
        global color_cache
        color_idx = (classes[j] * 5 if class_color else j * 5) % len(COLORS)
        
        device_key = str(on_gpu) if on_gpu is not None else 'cpu'
        if on_gpu is not None and color_idx in color_cache[device_key]:
            cached_color = color_cache[device_key][color_idx]
            if return_tensor:
                return cached_color
            else:
                # Convert tensor to tuple for OpenCV
                if isinstance(cached_color, torch.Tensor):
                    return tuple(int(c * 255) for c in cached_color.cpu().numpy())
                return cached_color
        else:
            color = COLORS[color_idx]
            if not undo_transform:
            ...
            if on_gpu is not None and return_tensor:
                color_tensor = torch.Tensor(color).to(on_gpu).float() / 255.
                color_cache[device_key][color_idx] = color_tensor
                return color_tensor
            elif on_gpu is not None:
                color_tensor = torch.Tensor(color).to(on_gpu).float() / 255.
                color_cache[device_key][color_idx] = color_tensor
                # Return as integer tuple for OpenCV
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        # colors = torch.cat([get_color(j, on_gpu=img_gpu.device).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
        colors = torch.cat([get_color(j, on_gpu=img_gpu.device, return_tensor=True).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
    ...
    #             color = tuple(int(c) for c in color.cpu().numpy())
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    if device.type == 'cuda' and torch.cuda.device_count() > 1:
        net = CustomDataParallel(net).cuda()
        print(f"Using {torch.cuda.device_count()} CUDA GPUs for video processing")
    else:
        net = net.to(device)
        if device.type == 'mps':
            print("Using MPS for video processing - DataParallel not supported")
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if torch.backends.mps.is_available():
            ...
            args.cuda = False  # MPS is not CUDA
            torch.set_default_tensor_type('torch.FloatTensor')
